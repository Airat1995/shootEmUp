...
IImage::IImage(ImageFormat format, ImageType type, ImageUsage usage, BufferStageFlag stage, int binding, int width, int height, int sampleCount, std::vector<unsigned char>& imageData)
...
IImage::IImage(ImageFormat format, ImageType type, ImageUsage usage, BufferStageFlag stage, int binding, int width,
               int height, int sampleCount): _width(width), _height(height), _binding(binding), _sampleCount(sampleCount), _imageData(), _format(format), _type(type), _usage(usage), _stage(stage)
{
