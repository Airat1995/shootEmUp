...
{
    VulkanFramebuffer::VulkanFramebuffer(VkDevice device, uint32_t grapQueueFI, uint32_t presentQueueFI,
                                         ISwapchain &swapchain,
                                         VulkanRenderpass &renderpass, VulkanCommandPool &commandPool,
                                         VulkanDepthBuffer &depthBuffer) : _device(device), _commandPool(commandPool),
    ...
    VulkanFramebuffer::VulkanFramebuffer(VkDevice device, uint32_t grapQueueFI, uint32_t presentQueueFI,
                                         ISwapchain &swapchain, BufferlessVulkanImage &image,
                                         VulkanRenderpass &renderpass, VulkanCommandPool &commandPool,
                                         VulkanDepthBuffer &depthBuffer) : _device(device), _commandPool(commandPool),
                                                                           _swapchain(swapchain), _renderpass(renderpass) {
        CreateQueues(device, grapQueueFI, presentQueueFI);
    ...
    void VulkanFramebuffer::SubmitFramebuffer(int index) {
        uint32_t imageIndex;
        auto result = vkAcquireNextImageKHR(_device, _swapchain.Swapchain(), UINT64_MAX,
                                            _swapchain.ImageAvailableSemaphore(), nullptr, &imageIndex);
        if (result != VK_SUCCESS) {
