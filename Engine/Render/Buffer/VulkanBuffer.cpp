...
{
    VulkanBuffer::VulkanBuffer(VkDevice device, VkPhysicalDevice physical, BufferStageFlag stageFlag, BufferUsageFlag usage,
    ...
    void VulkanBuffer::Fill() {
        const VkResult result = vkMapMemory(_device, _memory, 0, _memoryReq.size, 0,
    ...
    void VulkanBuffer::Destroy() const noexcept {
    ...
    const void *VulkanBuffer::DataLocation() const {
    ...
    uint32_t VulkanBuffer::FindMemoryType(uint32_t typeFilter, VkPhysicalDeviceMemoryProperties memoryProperties,
    ...
    VkDescriptorType VulkanBuffer::BufferUsageToDescriptorType(BufferUsageFlag bufferUsageFlag) {
        ...
        case BufferUsageFlag::UniformTexel:
            descriptor = VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
            break;
        case BufferUsageFlag::StorageTexel:
            descriptor = VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER;
            break;
        case BufferUsageFlag::UniformBuffer:
            descriptor = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            break;
        case BufferUsageFlag::StorageBuffer:
            descriptor = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            break;
        case BufferUsageFlag::TransferSrc:
            descriptor = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
            break;
        default:
            descriptor = (VkDescriptorType) bufferUsageFlag;
            cerr
                    << "Current type not supported! Please add new case statement in VulkanPipeline->BufferUsageToDescriptorType"
                    << endl;
            break;
        }
